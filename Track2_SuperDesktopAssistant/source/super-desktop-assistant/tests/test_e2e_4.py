"""E2E Test 4: Analyze PDF transcript."""
...
# 优先检查环境变量，否则使用默认测试路径（仅在文件存在时运行）
_TEST_PDF = os.environ.get("TEST_PDF_PATH", r"D:\QQ_download\梁文锋投资者交流会-录音转文本.pdf")
if not os.path.isfile(_TEST_PDF):
    _TEST_PDF = None  # 文件不存在，跳过 PDF 分析
TEST_PROMPT = (
    f'系统性分析这个"{_TEST_PDF}"' if _TEST_PDF
    else None  # test runner will skip
)


async def main():
    sp("=" * 60)
    sp("  Test 4: PDF Analysis")
    sp("=" * 60)

    if TEST_PROMPT is None:
        sp("[SKIP] No PDF file available. Set TEST_PDF_PATH env var or place the PDF at the default path.")
        return
